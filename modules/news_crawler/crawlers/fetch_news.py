...
def fetch_36kr():
    ...
    if html:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if title and 8 < len(title) < 60 and any(kw in title.upper() for kw in ['AI', '智能', '大模型', 'GPT', '英伟达', '特斯拉', '自动驾驶', '芯片', '科技', '创业', '融资']):
                if link.startswith('/'):
                    link = "https://36kr.com" + link
                elif link.startswith('//'):
                    link = "https:" + link
                news.append({"title": title, "link": link, "source": "36 氪", "priority": 12})
    
    print(f"36 氪：{len(news)}条")
    return news
...
def fetch_huxiu():
    """采集虎嗅网新闻"""
    news = []
    html = get_page("https://www.huxiu.com/")
    ...
    if html:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if title and 8 < len(title) < 60 and any(kw in title.upper() for kw in ['AI', '智能', '大模型', 'GPT', '英伟达', '特斯拉', '自动驾驶', '芯片', '科技']):
                ...
                news.append({"title": title, "link": link, "source": "虎嗅网", "priority": 10})
    
    print(f"虎嗅网：{len(news)}条")
    return news
...
def fetch_jqr():
    """采集机器之心新闻"""
    news = []
    html = get_page("https://www.jiqizhixin.com/")
    ...
    if html:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if title and 8 < len(title) < 60:
                ...
                news.append({"title": title, "link": link, "source": "机器之心", "priority": 15})
    
    print(f"机器之心：{len(news)}条")
    return news
...
def fetch_xz2():
    """采集新智元新闻"""
    news = []
    html = get_page("https://www.ai-era.com/")
...
def fetch_qbit():
    """采集量子位新闻"""
    news = []
    html = get_page("https://www.qbitai.com/")
...
def fetch_infoq():
    """采集 InfoQ 新闻"""
    news = []
    html = get_page("https://www.infoq.cn/")
    ...
    if html:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if title and 8 < len(title) < 60 and any(kw in title.upper() for kw in ['AI', '智能', '大模型', 'GPT', '架构', '技术', '开发']):
                if link.startswith('/'):
                    link = "https://www.infoq.cn" + link
                news.append({"title": title, "link": link, "source": "InfoQ", "priority": 10})
    
    print(f"InfoQ: {len(news)}条")
    return news
...
def fetch_csdn():
    """采集 CSDN 新闻"""
    news = []
    html = get_page("https://www.csdn.net/")
    ...
    if html:
        ...
        for a in soup.find_all('a', href=True):
            ...
            if title and 8 < len(title) < 60 and any(kw in title.upper() for kw in ['AI', '智能', '大模型', 'GPT', 'Python', '开发', '技术']):
                ...
                elif 'article' not in link and 'blog' not in link:
                    continue
                news.append({"title": title, "link": link, "source": "CSDN", "priority": 8})
    
    print(f"CSDN: {len(news)}条")
    return news
...
def fetch_news():
    ...
    try:
            ...
            if row:  # 更新
                c.execute('''UPDATE news SET 
                    ...
                    (summary, detail.get('image'), detail.get('content'),
                     ...
                     category, detail.get('time', datetime.now().strftime("%Y-%m-%d")),
                ...
                c.execute('''INSERT INTO news 
                    ...
                    (n['title'], n['link'], n['source'], category, 
                     detail.get('time', datetime.now().strftime("%Y-%m-%d")),
                     summary, detail.get('image'), detail.get('content'),
                     json.dumps(keywords, ensure_ascii=False),
                     json.dumps(entities, ensure_ascii=False),
                     hot_score, determine_trend_level(hot_score),
        ...
        c.execute('''UPDATE crawl_log SET 
                     ...
                     updated_news_count=?, status=? 
                     WHERE id=?''',
                  (end_time, len(unique), new_count, updated_count, 'completed', crawl_log_id))
        conn.commit()
        
        print(f"\n处理完成: {new_count}条新增, {updated_count}条更新, 共{len(unique)}条")
